...
# this function extracts all the text from each page and stores it in a string called text and returns it
...
# this function is responsible for converting the extracted chunks into vectore embeddings and storing it in a local folder called faiss_index
def get_vector_store(text_chunks):
    embeddings = GoogleGenerativeAIEmbeddings(model = "models/embedding-001")
    vector_store = FAISS.from_texts(text_chunks, embedding=embeddings)
    vector_store.save_local("faiss_index")

# this function defins the prompt template for the model, it defines the chain with the defined prompt, chaintype and model
...
# this function takes the user question as input and runs it throught the model and stores and prints the response
